...
def get_average_sentence_length(text):
    ...
    sentence_lengths = []
    for i in sentences:
        sentence_lengths.append(i.count(' ') + 1) # the number of words will always be 1 more than the number of whitespaces between words.
...
def prepare_text(text):
    ...
    text = text.replace('.', '') # remove fullstops
...
def build_frequency_table(corpus):
    frequency_table = {}
    unique_words = set(corpus)  # make a list of unique words in the corpus
...
def ngram_creator(text_list):
    ngram = []
    for i in range(0, len(text_list) - 1): # range goes to second last word in text_list
...
def frequency_comparison(table1, table2):
    ...
    return mutual_appearances / appearances # frequency comparison score
